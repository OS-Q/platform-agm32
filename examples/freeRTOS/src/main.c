...
/* Event notification and task for GPTIMER */
static TaskHandle_t xGPTimerTask;
static void prvGPTimerTask(void *pvParameters);
...
static volatile uint32_t ulCountOfGPTimer = 0;
...
static void prvSetupHardware(void)
{
  /* Setup the clocks, etc. here, if they were not configured before
     main() was called. */
  board_init();
  ...
  init.Autoreload = SYS_GetSysClkFreq() / 1000 * mainGPTIMER_PERIOD_MS;
...
void GPTIMER0_isr(void)
{
  ...
  // Notify the task
  vTaskNotifyGiveFromISR(xGPTimerTask, NULL);
}
